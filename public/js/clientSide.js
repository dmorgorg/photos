$(document).ready(function() {
    ...
    $('#tagging').change(function(){
        ...
        */
        var selected='<span style="color:black;">Tags: </span>';
        $('#tagging :selected').each(function(){
            selected += $(this).text() + ', ';
        });
        // remove trailing comma
        selected = selected.substr(0, selected.length-3);
        $('#tagged').html(selected);
    });
    ...
    $('#fileInput').change(function(){
       ...
       if((suffix === '.jpg')||(suffix === '.png')){
            if(len === 24){
                ...
                $('#renameOrWarn').html("Renamed file to: "+filename + " &nbsp; <input type='checkbox' checked='true'>");

                // also let's populate the date field from filename
                var date = filename.replace(filename.substring(10), '');
                $('#dateTaken').css('color', 'red');
                $('#dateTaken').val(date);
            }
       } else {
           $('#renameOrWarn').html("The site currently accepts only .jpg or .png files");
       }
    });
